<?php
/*
 * Rimarx Top Commentators Widget Class
 *
 *
 */
...
class Mynew_widget extends WP_Widget {
    ...
    protected function get_users_list( $user_role, $users_amount ){

        if ( $user_role == 'All' ) {
            $usersObj = get_users( array( 'number' => $users_amount, ) );
        }else{
            $usersObj = get_users( array( 'number' => $users_amount, 'role' => $user_role, ) );
        }
    ...
    public function widget( $args, $instance ) {
        ...
        $usersArr = $this->get_users_list( $instance['user_roles'], $instance['users_amount'] );

        for ( $i = 0; $i < $instance['users_amount']; $i++ ) { 
